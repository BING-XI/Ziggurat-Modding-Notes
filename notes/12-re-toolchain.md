...
| `0x5584B000` | `build_hero_turn1_upgrade.py` (🔨 APPLIED, UNTESTED 2026-09-22) | **exclusive**, `0x80` (40 B used — `C_TURN1` `0x5584B000`) | Map-placed heroes keep their unspent skill points and get a turn-1 level-up prompt. Replaces the **whole** 10-byte day-1 confiscation block in `THero.NewDay` at `0x55786CB3` (`mov eax,ebx / call C_TURN1 / 3× nop`) — nothing displaced, and both inbound jumps (`je` from `0x55786CA0`, `jne` from `0x55786CB1`) land on the run's boundaries, never inside. PIC for free: two rel32 calls (`GetSkillPoints @0x557875C4`, `GetLevel @0x55787740`), no absolute operand, no anchor. Clobbers EAX/EDX/ECX, preserves EBX. **Current high-water mark** |
