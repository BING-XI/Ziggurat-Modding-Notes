...
|---|---|---|---|
| `0x5580BFA0` | *(none — pre-existing Ziggurat hand-edit)*; retired by `build_pathoffrost_nolava.py` | 31 B | **DEAD** since 2026-09-24. Path of Frost's land arm plus lava 9 → wasteland 5, formerly `call`ed from `0x55780231`; the script restored the vanilla arm there. ⚠ Not free space: that script's `--undo` re-links it. |
...
| `0x5580C240` | *(none — pre-existing Ziggurat hand-edit)*, formalised/extended by `build_marksmanship_atk2.py` (not applied); its 4-byte level test at `0x5580C27F` is owned by `build_firmament_rangedmalus.py` | — | Marksmanship ranged rework + Cave/Depths height malus. One of the caves found by the 2026-07-08 diff sweep with **no owning script or doc** — five years of hand edits, not corruption; ask the author rather than reverse-engineering intent (§2). |
...
| `0x5580DB40` | `build_icestorm_lava.py` (**current, live**) — `cave_iceroll`, exactly 32 B, ends exactly at `0x5580DB60` | 0x20 | per-proc gate; since 2026-09-25 `build_icestorm_gate25.py` flips its `je` at `0x5580DB4E` so a proc takes effect 25% of the time (it was: 25% chance of no change). **⚠ COLLISION, not live**: `build_raiseterrain_mtn.py` also declares `CAVE = 0x5580DB40` ("zero space, above cave_icelava"), written when `cave_iceroll` did not yet exist. That script is doubly dead — SHELVED since 2026-07-07 because its own premise was disproved in-game, *and* its target address is now occupied. Its own verify-before-write would abort cleanly if ever run, but **never apply it**. See §5.7. |
...
| `0x5584D240` | `build_vortex_reseed.py` (🔨 APPLIED, UNTESTED 2026-09-24) | **exclusive**, `0x80` (74 B) | Vortex re-seed (`04-spells-modded.md`). Hook `0x557A150C` (5 B) |
| `0x5584D2C0` | `build_spell_family.py` (🔨 APPLIED, UNTESTED 2026-09-25) | **exclusive**, `0x40` (`cave_rw` 34 B) | `Spells.pfs` tag `0x12` → spell byte `+0x23` (`03-abilities-added.md`, Caster Cost). Hook `0x557792BB` (8 B, `TSpell.ReadWrite` tail) |
...
| `0x5584D900` | `build_group_move_map.py` (🔨 APPLIED, UNTESTED 2026-09-25) | **exclusive**, `0x200` (`cave_order` 143 B, `cave_call` `0x5584D990` 66 B, `cave_over` `0x5584D9E0` ~120 B) | world-map group move. Hooks `0x55792CE5` (8 B), `0x55792D07` (10 B), `0x55790E48` (6 B, `TArmyHS.CanMoveOver` entry). **BSS `G_SEL 0x558FAD10`, `G_IDX 0x558FAD14`** |
| `0x5584DB00` | `build_weakness.py` (🔨 APPLIED, UNTESTED 2026-09-25) | **exclusive**, `0x400` (`weak_mask`, two strike caves, `cave_strat`, `cave_resist`, six `cave_eff_*`, `cave_reg` `0x5584DD40` 424 B with the seven name literals) | Weakness abilities `0xB3`–`0xB9`. Hooks `0x557BCDFB` (5 B, registration splice), `0x55726A43`/`0x55726AD8` (6 B), `0x55781B14`/`0x55781B84` (10 B), six effect sites (7 B) **⚠ COUPLED with `build_embrittle.py`**: `S1`/`S2` overwrite the trailing `nop` of Embrittle's 10-byte hooks at `0x55726A3A`/`0x55726ACF` (`0x55726A43`, `0x55726AD8`), so `build_embrittle.py` reads MIXED/FOREIGN while Weakness is installed and its `--undo` aborts. Undo `build_weakness.py` first. The lightning effect hook `0x55781C90` also covers bytes `build_lightning_ignites.py` guards read-only, so that script's dry run aborts "bytes this patch depends on have moved" while Weakness is installed; its installed bytes are unaffected. |
| `0x5584DF00` | `build_lethargy.py` (🔨 APPLIED, UNTESTED 2026-09-25) | **exclusive**, `0x300` (`cave_moves` 30 B, `cave_str` `0x5584DF20` 125 B, `cave_stru` `0x5584DFA0` 125 B, `cave_cur` `0x5584E020` 55 B, `cave_fcval` `0x5584E060` 166 B, `cave_tcval` `0x5584E110` 166 B) | Slow → Lethargy, and Haste's extra strike (`04-spells-modded.md`). Hooks `0x55724FEE` (5 B jmp), call retargets `0x55767DE7`/`0x55767B15`/`0x557F8868`, `0x5577FF9E` (1 B), `TSlow` VMT slots `0x557F4E98` and `0x557F4E84` (`.reloc` kept). One call/pop anchor for `TFastCombatUnit`'s VMT |
| `0x5584E200` | `build_liquidbody.py` (🔨 APPLIED, UNTESTED 2026-09-25) | **exclusive**, `0x200` (`cave_or` 28 B, `cave_burn` `0x5584E220` 37 B, `cave_reg` `0x5584E250` 64 B incl. the name) | Liquid Body `0xBA` (`03-abilities-added.md`). Hooks `0x557BCDC6` (registration splice), `0x5574F80B`/`0x5574F9A3` (6 B each), `0x55781F0D` (5 B). One call/pop anchor |
...
(`0x55844000`, `0x55848000`, `0x55849000`, `0x5584A000`, `0x5584B000`, `0x5584C000` and `0x5584D000`
are now claimed — see the rows above. `0x55848400..0x55849000` is the gap between
...
| `0x0058F100..0x0058F13F` | `AoWDevEd.exe` (`.ctp`, above heroprune's `ZONE_CEIL`) | `build_deved_valcircle.py` | **exclusive**, 37 B used (2026-09-25) — `cave_circle`: Map Validation double-click also plays the shrinking circle. Reached by retargeting the `.vgo` cave's `call CenterView` at `0x005900E4`. `08-editor.md` §16.4 |
...
✅ **Fixed 2026-09-25: `build_deved_heroprune.py` no longer destroys `build_deved_gamesettings_tab.py`'s cave.** Until then its `strip()` zeroed `.ctp`'s tail to the section end on both `--apply` and `--undo`, wiping `0x0058F280..0x0058F3FF` and leaving four `call rel32`s (one in `TMainForm.FormCreate`) pointing at zeros, an AV at editor startup. It now stops at `ZONE_CEIL 0x0058F100`; a full `--undo`/`--apply` round trip left gamesettings verified intact.
...
⚠ **`.ctp`'s first dead `TMainForm` method table is now heroprune's code.** `0x0052E060..0x0052ECF2` (terrainpal's 134 entries, 3219 B) holds both prune handlers while heroprune is applied; its `--undo` re-emits the table there before pointing VMT−0x28 back at it. Do not allocate in it. The genuinely reclaimable dead run is the pre-terrainpal table.
...
| `0x00438800` | `build_ai_ram_breach.py` | tactical AI stops focusing harmless wall crushers after a breach (`10-ai-and-structures.md` §10.2). Hook `0x413673` (8 B). Cave 204 B, `0x438800`–`0x4388CB`; zone **`0x438800`–`0x438BFF`** reserved to this script |
| `0x00438C00` | `build_ai_ranged_walkvalue.py` | tactical AI archers stop forfeiting a shot for a marginally clearer hex (`10-ai-and-structures.md` §10.3). Hook `0x4179CE` (13 B) plus in-place `0x417D1D` (2 B) and `0x417E27` (1 B). Fixed slots: `0x438C00` h2 39 B, `0x438C40` udist 63 B, `0x438C80` disc 370 B (ends `0x438DF2`); `--undo` zeroes `0x438C00`–`0x438DFF`. Zone **`0x438C00`–`0x4393FF`** reserved to this script |
...
| `0x59824800` | `build_wheel_combo.py` | wheel scrolls an open `TAOWComboBox` (`07-ui.md` §17): `cave_setc` 41 B, `WheelCombo` `0x59824840` 93 B → header slot 3 `0x5982401C`, latch[7] `0x5983E05C`. Zone `0x59824800`–`0x598248FF`; CODE is zero from `0x598248A0` to `0x5983D0E4` (~98 KB) |
...
⚠ **Saved page edits are keyed by list index** (`NOTE.<slug>.<i>`, `MAJOR.<i>.0`, `LATEST.<i>` … in
`manual_text.json`), so inserting an entry above an edited one silently moves the edit onto its
neighbour. On 2026-09-25 five `SPELL_BEHAVIOUR` inserts moved the Sphere Mastery line from index 7
to 12 while its edit stayed keyed at 7, so the Raise Terrain underground line now at 7 vanished
under it and Sphere Mastery printed twice.
Append, or re-key the JSON in the same edit. The text diff of the new page against
`gh-repo/index.html` before a publish is what caught it.
