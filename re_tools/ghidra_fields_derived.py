"""Bulk-DERIVED field entries -- generated, kept separate from the hand-curated core.

Produced 2026-08-06 by 17 agents that were given ONLY offsets, never the proposed
meanings, so that agreement with the earlier pass is evidence rather than assent.
Against 122 overlapping offsets the two independent derivations produced ZERO
contradictions -- 96 outright agreements, the rest agreement-with-refinement, and 5
where both honestly returned UNKNOWN.

Evidence: M = agent reported CERTAIN with a function symbol; D = PROBABLE; S = UNKNOWN.
`ghidra_fields.py` merges this in and bounds/inheritance-checks it like anything else.
"""
...
DERIVED = {
    'TAbility (AoWE unit, AoWEPACK.dpl; VMT @ 0x5570F254, class name ptr [VMT-0x20]=0x5570F372 "TAbility", instance size [VMT-0x1C] = 0x24, parent cell [VMT-0x18]=0x558FC92C -> imported Engine.TEObject)': [
        ...
        (0x08, 0, 'fname_the_ability_s_display_name_a', 'M', "FName - the ability's display name, as a reference-counted Delphi AnsiString. Returned verbatim by GetName and ExpandName; set in each concrete abilit"),
        (0x0C, 0, 'fabilityid_the_ability_s_global_ab', 'M', "FAbilityID - the ability's global ability id (0..N). It is the index under which the singleton is stored in TAbilityControl's TList, the key every TAb"),
        (0x10, 0, 'fdescription_a_tstringlist_holding', 'M', "FDescription - a TStringList holding the ability's description text lines. Created empty in the constructor, freed in the destructor, filled from the "),
        (0x14, 0, 'fexpandcost_the_point_cost_of_acqu', 'M', "FExpandCost - the point cost of acquiring this ability, returned unchanged by ExpandCost, and also returned by GetSkillPoints as the ability's skill-p"),
        (0x18, 0, 'fsfx_the_ability_s_sound_effect_li', 'M', "FSFX - the ability's sound-effect library: the set of sounds played when the ability fires. Constructed empty, freed in the destructor, populated from"),
        (0x1C, 0, 'fanimation_the_ability_s_image_seq', 'M', "FAnimation - the ability's image-sequence list (its on-screen effect animation, and the frames the ability draws over its target). Created against the"),
        (0x20, 0, 'fselectiontypes_the_set_of_owner_c', 'M', "FSelectionTypes - the set of owner categories this ability may legitimately belong to / be selected for. Members (from the enum's own RTTI, ordinal = "),
    ],
    'TAbstractUnit': [
    ...
    'TAoWHexagon': [
        (0x0C, 0, 'fstatus_tmapobjectstatus_a_1_byte_', 'M', 'FStatus : TMapObjectStatus - a 1-byte Delphi SET of runtime state flags (RTTI-confirmed type name and member names). Bits: 0x01 moVisible; 0x02 moReso'),
    ],
    'TAoWWaterHexagon': [
    ...
    'TStructure': [
        ...
        (0x10, 0, 'hex_x_coordinate_hx_x_of_the_objec', 'M', 'Hex X coordinate (HX x) of the object on the map - signed byte. Feeds every hex-geometry call the class makes.'),
        (0x11, 0, 'hex_y_coordinate_hx_y_of_the_objec', 'M', 'Hex Y coordinate (HX y) of the object on the map - signed byte. Always used as the second argument alongside +0x10.'),
        ...
        (0x18, 0, 'terrain_type_the_object_currently_', 'M', 'Terrain type the object currently sits on / renders for - signed byte (negative = none/invalid). Set from the terrain-changed notification and from th'),
    ...
    'TUnitResource': [
        (0x1C, 0, 'the_unit_type_s_name_a_delphi_ansi', 'M', "The unit type's name - a Delphi AnsiString (long-string pointer). It is the base component of the displayed unit name: TUnitResource.GetUnitName assig"),
        ...
        (0x52, 0, 'not_a_field_trailing_alignment_pad', 'D', 'Not a field - trailing alignment padding in the instance. The last real field is the 1-byte 0x51, so the declared layout ends at 0x52, and Delphi roun'),
    ],
}
